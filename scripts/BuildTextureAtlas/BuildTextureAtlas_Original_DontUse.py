...
def addFolder(folder):
  ...
  folderName = folder + '/'
  fileName = str(originalIndex) + '.png'
  while os.path.isfile(folderName + fileName) :
    print(folderName + fileName)
    images.append(SourceImage(folderName, fileName, originalIndex))
...
print('Computing... Dont close the window')

# Sort the source images using the insert heuristic.
images.sort(key=maxExtent)
...
print('Writing into the output... Dont close the window')
...
images.sort(key=lambda x: x.origIndex)
