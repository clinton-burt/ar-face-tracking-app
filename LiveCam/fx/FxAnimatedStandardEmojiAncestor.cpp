...
OpenGLAnimatedStandardEmojiModel::OpenGLAnimatedStandardEmojiModel()
{
...
void OpenGLAnimatedStandardEmojiModel::loadFromJSON(boost::property_tree::ptree& modelRecord)
{
	OpenGLModel::loadFromJSON(modelRecord);
}

void OpenGLAnimatedStandardEmojiModel::applySuit(boost::property_tree::ptree& suit, size_t faceIndex, TCommonRenderParams& commonRenderParams,
	bool loadTexturesImmediately)
{
	...
	animationState[faceIndex] = OPENING_ANIM;
	...
	animationBackward[faceIndex] = suit.get<bool>("animationBackward", true);
...
bool OpenGLAnimatedStandardEmojiModel::load()
{
...
void OpenGLAnimatedStandardEmojiModel::transform(FXModel& face, ExternalRenderParams &externalRenderParams)
{
...
void OpenGLAnimatedStandardEmojiModel::draw(FXModel& face, ExternalRenderParams &externalRenderParams)
{
...
void OpenGLAnimatedStandardEmojiModel::unload()
{
	OpenGLModel::unload();
...
void OpenGLAnimatedStandardEmojiModel::animate(FXModel& face)
{
	// animation from the begin to the end, and than if animationBackward is true - from the end to the begin.
	
	size_t faceIndex = face.pointId;

	if (animationState[faceIndex] == OPENING_ANIM)
	{
		...
		if (animationIndexes[faceIndex] > animationLength[faceIndex] - 1e-2f)
		{
			...
			{
				animationIndexes[faceIndex] = animationLength[faceIndex] - 1;
				animationState[faceIndex] = CLOSING_ANIM;
			}
			else
			{
				animationIndexes[faceIndex] -= animationLength[faceIndex];
			}
		}
	}
	if (animationState[faceIndex] == CLOSING_ANIM)
	{
		animationIndexes[faceIndex] -= animationSpeed[faceIndex];
		if (animationIndexes[faceIndex] <= 1e-2f)
		{
			animationIndexes[faceIndex] = 0;
			animationState[faceIndex] = OPENING_ANIM;
		}
...
std::shared_ptr<ObjectRenderParams> OpenGLAnimatedStandardEmojiModel::createDefaultObjectRenderParams()
{
...
void OpenGLAnimatedStandardEmojiModel::setDefaultObjectRenderParams(ObjectRenderParams& params)
{
	params.blend = true;
...
AtlasAnimatedEmojiModel::AtlasAnimatedEmojiModel()
{
}

void AtlasAnimatedEmojiModel::applySuit(boost::property_tree::ptree& suit, size_t faceIndex, TCommonRenderParams& commonRenderParams,
	bool loadTexturesImmediately)
{
	OpenGLModel::applySuit(suit, faceIndex, commonRenderParams, loadTexturesImmediately);
	...
	animationPaths[faceIndex] = suit.get<std::string>("animationAtlasPath", "");

	//animationOpened[faceIndex] = suit.get<size_t>("animationOpened", 1);

	if (loadTexturesImmediately)
	{
		auto loadingResult = resourceManager.loadAtlas(animationPaths[faceIndex]);

		atlasTextureScale[faceIndex] = loadingResult.atlasTextureSize;
		atlasTextures[faceIndex] = loadingResult.atlasTextures;
		animationLength[faceIndex] = loadingResult.count;

		animationAtlasID[faceIndex] = loadingResult.ID;
		animationSize[faceIndex] = { loadingResult.atlasWidth, loadingResult.atlasHeight };
	}
...
bool AtlasAnimatedEmojiModel::load()
{
	objects.push_back(ModelLoader::CreateQuadModel());
...
void AtlasAnimatedEmojiModel::transform(FXModel& face, ExternalRenderParams &externalRenderParams)
{
	double vAngle = externalRenderParams.vAngle;
	...
	case TransformVersion::V1:
	{
		...
		modelMatrix *= Affine3d(Translation3d(Vector3d((XMirror ? -1 : 1) * animationOffset[face.pointId][0],
			animationOffset[face.pointId][1], animationOffset[face.pointId][2]))).matrix();
		double Xscale = face.faceWidth * animationScale[face.pointId][0] * (XMirror ? -1 : 1);
		double Yscale = face.faceWidth * animationScale[face.pointId][1] * atlasTextureScale[face.pointId][1] * animationSize[face.pointId][1] /
			animationSize[face.pointId][0] / atlasTextureScale[face.pointId][0];
		modelMatrix *= Affine3d(Scaling(Xscale, Yscale, 1.)).matrix();
		depth = -h - faceDepth;
		renderParams.modelView = modelMatrix.cast<float>();
		break;
...
void AtlasAnimatedEmojiModel::draw(FXModel& face, ExternalRenderParams &externalRenderParams)
{
	...
	//shader->setUniform2f("textureScale", rect.atlasRect.width, rect.atlasRect.height);
	//shader->setUniform2f("textureShift", rect.atlasRect.x, rect.atlasRect.y);
	//shader->setUniform2f("leftTopCrop", rect.leftTopPadding[0], rect.leftTopPadding[1]);
	//shader->setUniform2f("rightBottomCrop", rect.rightBottomPadding[0], rect.rightBottomPadding[1]);
...
void AtlasAnimatedEmojiModel::unload()
{
	OpenGLModel::unload();
...
AtlasAnimatedToungeEmojiModel::AtlasAnimatedToungeEmojiModel()
{
}

void AtlasAnimatedToungeEmojiModel::applySuit(boost::property_tree::ptree& suit, size_t faceIndex, TCommonRenderParams& commonRenderParams,
	bool loadTexturesImmediately)
{
	OpenGLModel::applySuit(suit, faceIndex, commonRenderParams, loadTexturesImmediately);
	...
	animationIndexes[faceIndex] = 0;
	animationState[faceIndex] = CLOSED_ANIM;
...
bool AtlasAnimatedToungeEmojiModel::load()
{
	objects.push_back(ModelLoader::CreateQuadModel());
...
void AtlasAnimatedToungeEmojiModel::transform(FXModel& face, ExternalRenderParams &externalRenderParams)
{
	double vAngle = externalRenderParams.vAngle;
...
void AtlasAnimatedToungeEmojiModel::draw(FXModel& face, ExternalRenderParams &externalRenderParams)
{
	if (animationIndexes[face.pointId] > animationLength[face.pointId] - 2e-2)
	{
		return;
...
void AtlasAnimatedToungeEmojiModel::unload()
{
	OpenGLModel::unload();
...
void AtlasAnimatedToungeEmojiModel::animate(FXModel& face)
{
	...
	else if (animationState[faceIndex] == CLOSING_ANIM)
	{
		animationIndexes[faceIndex] += animationSpeed[faceIndex];
		if (animationIndexes[faceIndex] > animationLength[faceIndex] - 1e-2f)
		{
