#include <fx/FxFaceSwapSjSharksAncestor.h>
...
FxFaceSwapSjSharksAncestor::FxFaceSwapSjSharksAncestor()
{
	//Do this in subclass, not in FxFaceSwapSjSharksAncestor class constructor
...
cv::Point2i FxFaceSwapSjSharksAncestor::getPoint(int i, const std::array<float, 66 * 2>& pts)
{
	return cv::Point2i(pts[(i - 1) * 2], pts[(i - 1) * 2 + 1]);
}

void FxFaceSwapSjSharksAncestor::applyModule(FilterModule *module, size_t faceIndex, bool loadTExturesImmediately)
{
...
void FxFaceSwapSjSharksAncestor::selection(QString absolutePath, int index)
{
	int tryCounter = 0;
...
void FxFaceSwapSjSharksAncestor::drawOnCVFrame_OLD(cv::Mat frame, FXModel& fxModel) {
...
void FxFaceSwapSjSharksAncestor::featherMask(cv::Mat &refined_masks)
{
...
inline void FxFaceSwapSjSharksAncestor::pasteFacesOnFrame(cv::Mat frameVal)
{
...
void FxFaceSwapSjSharksAncestor::specifiyHistogram(const cv::Mat source_image, cv::Mat target_image, cv::Mat mask)
{
	uint8_t LUT[3][256];
